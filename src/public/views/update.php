        ...
        <div class="box-new-ticket">
            <form action="" method="post">
                <div class="new-ticket">
                    ...
                    <input type="text" placeholder="Entrez l'objet de votre ticket" name="title" required id="title" value="<?= $ticket['tic_title'] ?>">
                    ...
                    <select id="category" name="category">
                        ...
                        <?php foreach ($categories as $category) : ?>
                            <option value="<?=$category['cat_id']?>" <?= ($ticket['category_id'] == $category['cat_id']) ? "selected" : "" ?> ><?=$category['cat_name']?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <select id="problem" name="problem">
                        ...
                        <?php foreach ($labels as $label) : ?>
                            <option value="<?=$label['lab_id']?>" <?= ($ticket['label_id'] == $label['lab_id']) ? "selected" : "" ?>><?= $label['lab_name']?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <select id="priority" name="priority">
                        ...
                        <?php foreach ($priorities as $priority) : ?>
                            <option value="<?= $priority['pri_id'] ?>" <?= ($ticket['priority_id'] == $priority['pri_id']) ? "selected" : "" ?>><?= $priority['pri_name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <textarea placeholder="Entrez la description de votre ticket" name="description" required rows="5"
                              cols="33" id="description"
                    ><?= $ticket['tic_description'] ?></textarea>
